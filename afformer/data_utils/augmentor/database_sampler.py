...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names):
        ...
        self.painting = sampler_cfg.get('PAINTING', None)
        if self.painting:
    ...
    def __del__(self):
        if self.use_shared_memory:
            print('Deleting GT database from shared memory')
            ...
            if num_gpus > 1:
                dist.barrier()
            print('GT database has been removed from shared memory')

    def load_db_to_shared_memory(self):
        print('Loading GT database to shared memory')
        ...
        if num_gpus > 1:
            dist.barrier()
        print('GT database has been saved to shared memory')
        return sa_key
    ...
    def filter_by_difficulty(self, db_infos, removed_difficulty):
        ...
        for key, dinfos in db_infos.items():
            ...
            print('Database filter by difficulty %s: %d => %d' % (key, pre_len, len(new_db_infos[key])))
        return new_db_infos
    ...
    def filter_by_min_points(self, db_infos, min_gt_points_list):
        for name_num in min_gt_points_list:
            ...
            if min_num > 0 and name in db_infos.keys():
                ...
                print('Database filter by min points %s: %d => %d' %
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                if self.painting:
                    ...
                    labels_2d = np.zeros((obj_points.shape[0], len(self.detection_mapping)+1), np.float32)
                    labels_2d[:, self.detection_mapping[class_name]] = 1
                    obj_points =  np.concatenate([obj_points, labels_2d], axis=1)

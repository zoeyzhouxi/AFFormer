...
import numpy as np
from afformer.loss.point_pillar_loss import PointPillarLoss

class PointPillarTomvLoss(PointPillarLoss):
    def __init__(self, args):
        super(PointPillarTomvLoss, self).__init__(args)
        self.kd = args['kd']
    ...
    def forward(self, output_dict, target_dict):
        ...
        # student_embedding = embedding.permute(0,2,3,1).reshape(N*H*W, C)
        kd_loss_feature = kl_loss_mean(
                F.log_softmax(student_feature, dim=1), F.softmax(teacher_feature, dim=1)
            )
        # kd_loss_embedding = kl_loss_mean(
        #         F.log_softmax(student_embedding, dim=1), F.softmax(teacher_feature, dim=1)
        #     )

        kd_loss = kd_loss_feature
        
        # kd_loss = kd_loss_feature + kd_loss_embedding
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None, suffix=''):
        """
        ...
        if pbar is None:
            print("[epoch %d][%d/%d] || Loss: %.4f || Conf Loss: %.4f"
                  " || Loc Loss: %.4f || Dir Loss: %.4f || KD Loss: %.4f" % (

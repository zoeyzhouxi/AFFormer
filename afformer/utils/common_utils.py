...
def compute_iou(box, boxes):
    ...
    # if union area is zero (degenerate or invalid polygon), define IoU as 0.0
    ious = []
    for b in boxes:
        try:
            union_area = box.union(b).area
            if union_area <= 0 or np.isnan(union_area):
                ious.append(0.0)
                continue
            inter_area = box.intersection(b).area
            if np.isnan(inter_area) or inter_area <= 0:
                ious.append(0.0)
                continue
            ious.append(float(inter_area / union_area))
        except Exception:
            # In case shapely encounters a numerical issue, fall back to 0 IoU
            ious.append(0.0)
...
def torch_tensor_to_numpy(torch_tensor):
    ...
    return torch_tensor.detach().numpy() if not torch_tensor.is_cuda else \
        torch_tensor.cpu().detach().numpy()

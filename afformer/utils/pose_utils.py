...
def add_noise_data_dict(data_dict, noise_setting):
    ...
    if noise_setting['add_noise']:
        for cav_id, cav_content in data_dict.items():
            for k in range(len(cav_content['k_frames'])):
                cav_content['k_frames'][k]['params']['lidar_pose_clean'] = cav_content['k_frames'][k]['params']['lidar_pose'] # 6 dof pose

                if "laplace" in noise_setting['args'].keys() and noise_setting['args']['laplace'] is True:
                    cav_content['k_frames'][k]['params']['lidar_pose'] = cav_content['k_frames'][k]['params']['lidar_pose'] + \

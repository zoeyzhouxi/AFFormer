...
from afformer.utils.common_utils import check_numpy_to_torch
import math
...
def get_pairwise_transformation(base_data_dict, max_cav, proj_first):
    ...
    k_frame_num = len(base_data_dict[list(base_data_dict.keys())[0]]['k_frames'])
    pairwise_t_matrix_dict = []
    if proj_first:
        # if lidar projected to ego first, then the pairwise matrix
        # becomes identity
        # no need to warp again in fusion time.
        for k in range(k_frame_num):
            pairwise_t_matrix_dict.append(pairwise_t_matrix)
    else:
        # Organize transformations by frame index first
        frame_t_lists = [[] for _ in range(k_frame_num)]
        
        # Group transformations by frame index
        for cav_id, cav_content in base_data_dict.items():
            for k in range(k_frame_num):
                lidar_pose = cav_content['k_frames'][k]['params']['lidar_pose']
                frame_t_lists[k].append(x_to_world(lidar_pose))  # Twx

        # Calculate pairwise transformations for each frame
        for k in range(k_frame_num):
            t_list = frame_t_lists[k]
            num_cavs = len(t_list)

            for i in range(num_cavs):
                for j in range(num_cavs):
                    if i != j:
                        # i->j: Ti
    ...
    return np.array(pairwise_t_matrix_dict) # [k, L, L, 4, 4]

...
import afformer.visualization.simple_plot3d.canvas_3d as canvas_3d
import afformer.visualization.simple_plot3d.canvas_bev as canvas_bev
...
def visualize(infer_result, pcd, pc_range, save_path, method='3d', left_hand=False):
        ...
        if method == 'bev':
            ...
            canvas.draw_canvas_points(canvas_xy[valid_mask], colors= (46, 117, 182), radius=1) # Only draw valid points
            if gt_box_tensor is not None:
                canvas.draw_boxes(gt_box_np,colors=(0,255,0)) # green
            if pred_box_tensor is not None:
                canvas.draw_boxes(pred_box_np, colors=(255,0,0)) # red
        ...
        plt.tight_layout()
        plt.savefig(save_path, transparent=False, dpi=600, bbox_inches='tight', pad_inches=0)
        plt.clf()
        plt.close()

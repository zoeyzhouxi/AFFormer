...
from afformer.utils import common_utils
...
# added by Xi Zhou
def visualize_feature_map(feature_map, feature_name, save_path, fused=True):
    """
    Visualize multiple channels of a feature map in a grid of subplots.
    
    Args:
        feature_map: PyTorch tensor or NumPy array of shape (C, H, W) or (B, C, H, W)
        name: String for the output file name
        num_channels: Maximum number of channels to visualize (default: 8)
    """
    # Handle PyTorch tensor case
    if isinstance(feature_map, torch.Tensor):
        if feature_map.dim() == 4:  # (B, C, H, W) -> take first batch
            feature_map = feature_map[0]
        elif feature_map.dim() != 3:  # Expect (C, H, W)
            raise ValueError(f"Expected tensor of shape (C, H, W) or (B, C, H, W), got {feature_map.shape}")
        # Move to CPU, detach from graph, and convert to NumPy
        feature_map = feature_map.cpu().detach().numpy()
    elif not isinstance(feature_map, np.ndarray):
        raise TypeError("feature_map must be a PyTorch tensor or NumPy array")

    if fused:
        avg_feature = feature_map.mean(axis=0)  # shape: (H, W)
        plt.imshow(avg_feature, cmap='winter')
    else:
        max_channel_index = feature_map.mean(axis=(1, 2)).argmax()
        plt.imshow(feature_map[max_channel_index], cmap='winter')

    plt.axis('off')
    plt.tight_layout()
    plt.savefig(f'{save_path}/{feature_name}.png', bbox_inches='tight', pad_inches=0)
    plt.close()